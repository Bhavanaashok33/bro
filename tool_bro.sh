...
sudo apt-get -y install gcc bison cmake flex g++ gdb make libmagic-dev libpcap-dev libgeoip-dev libssl-dev python-dev swig zlib1g-dev git vim python-pip libgoogle-perftools-dev libnuma-dev build-essential
...
sudo ln -s /data/bro-logs /usr/local/bro/logs
...
sudo cp $SRC_DIR/logstash/logstash.conf  /etc/logstash/conf.d/logstash.conf
